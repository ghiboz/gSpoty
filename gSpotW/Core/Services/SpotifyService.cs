...
public class SpotifyService : SceneSingleton<SpotifyService>
{
    ...
    {
        _authMethodConfig = new AuthorizationConfig();
        StartService();
    ...
    public void StartService(bool promptAuth = false)
    {
        if (_authenticator == null)
        {
            switch (AuthType)
            {
                case AuthenticationType.PKCE:
                    _authenticator = new PKCE_Authentification();
                    break;
                case AuthenticationType.ImplicitGrant:
                    _authenticator = new ImplicitGrant_Authentification();
                    break;
                case AuthenticationType.ClientCredentials:
                    _authenticator = new ClientCredentials_Authorization();
                    break;
                default:
                    break;
            }
        ...
        if (_authenticator != null)
        {
            ...
            _authMethodConfig = JsonConvert.DeserializeObject<AuthorizationConfig>(cfg);
    ...
    private async void OnAuthenticatorComplete(object authObject)
    {
        if (authObject != null)
        {
            ...
            {
                Console.WriteLine("Auth complete object is unknown. Authentification failed.");
                return;
            }
            ...
            if (_client != null)
            {
                ...
                {
                    OnClientConnectionChanged?.Invoke(_client);
                    Console.WriteLine($"Successfully connected to Spotify using '{AuthType}' authentificiation");
                };
            ...
            {
                Console.WriteLine("Unknown error creating SpotifyAPI client");
            }
        }
        else
        {
            Console.WriteLine($"Authenticator '{AuthType}' is complete but not provided a valid authenticator");
        }
    }
    ...
    private async System.Threading.Tasks.Task SendValidationRequest()
    {
        if (_client != null)
        {
            try
            {
                ...
                {
                    Console.WriteLine("Confirmation request is null");
                }
            }
            catch (Exception e)
            {
                Console.WriteLine($"Confirmation request exception: {e.ToString()}");
            }
        }
    }
