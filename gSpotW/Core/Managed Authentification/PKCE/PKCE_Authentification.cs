...
/// </summary>
public class PKCE_Authentification : IServiceAuthenticator
{
    ...
    public void Configure(object config)
    {
        ...
        {
            PKCEConfig = new PKCE_AuthConfig();
        }
    }
        ...
        if (HasPreviousAuthentification())
        {
            ...
            if (_pkceToken != null)
            {
                ...
                if (!_pkceToken.IsExpired)
                {
                    ...
                    Console.WriteLine($"PKCE token loaded | Expires at '{expireDT.ToLocalTime()}'");
                }
            }
        }
        else
    ...
    private async void GetFreshAuth()
    {
        ...
        {
            Console.WriteLine($"Exception opening browser for auth: '{e.ToString()}'");
        }
    }
    ...
    private async Task OnAuthCodeRecieved(object sender, AuthorizationCodeResponse response, string verifier)
    {
        ...
        if (response != null && !string.IsNullOrEmpty(response.Code))
        {
            ...
            Console.WriteLine("PKCE: Recieved Auth Code");
            SetAuthenticator(_pkceToken);
        }
    }
    ...
    private void OnTokenRefreshed(object sender, PKCETokenResponse token)
    {
        ...
        Console.WriteLine($"PKCE token refreshed | Expires at '{expireDT.ToLocalTime()}'");
        ...
        if (triggerEvent)
        {
            Console.WriteLine("PKCE: Success in refreshing expired token into new token");
            OnAuthenticatorComplete?.Invoke(_pkceAuthenticator);
        }
    }
